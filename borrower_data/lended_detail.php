    ...
    <body>
        ...
        <div class="container well well-sm" style="margin-left: 17%; width: 81%">
            ...
            <?php
               ...
               $sql = "SELECT * FROM borrower_data WHERE bo_returned_date = '0' ORDER BY bo_id DESC LIMIT $start_from, ".$results_per_page;
               $result = $conn->query($sql); 

                    echo "<div class='table-responsive'>";
                    echo "<table class='table' style='font-size: 15px; width: 98%'>";
                    echo "<thead>";
                    echo "<tr class='info'><th>Name</th><th>NIC</th><th>Item Code</th>"
                       . "<th>Item Name</th><th>ISBN</th><th>Lended</th><th>Return</th><th>Action</th>";
                    echo "</thead>";
                     while($row = $result->fetch_assoc()){
                            
                            //$member_name = "";
                            $member_nic  = $row["bo_customer_id"];//get member nic no...
                            $return_date =$row["bo_return_date"];//get return date...
                            
                            //Get member name from member table...
                            $sql2 ="SELECT * FROM member WHERE m_nic = '$member_nic'";
                            $result2 = $conn->query($sql2);
                            while ($row2 = $result2->fetch_assoc()) {
                                $member_name = $row2["m_first_name"] ." ". $row2["m_last_name"]; 
                            }
                             echo "<tbody>";
                             if($current_date > $return_date){
                                 echo "<tr style='background-color:#ff9999;'><td>";
                             }else if ($current_date < $return_date){
                                 echo "<tr style='background-color:#00e6ac;'><td>";
                             }else{
                                 echo "<tr style='background-color:#ffb84d;'><td>";
                             }
                             ...
                             ?>
                               <button class='btn btn-sm btn-info' type='submit' name="user_nic" value="<?php echo $row["bo_customer_id"] ?>">Send <i class='fa fa-envelope-o'></i></button>
                            <?php
                             echo "</form>";
                             echo "</td>";
                             echo "</tr></table>";
